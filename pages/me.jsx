...
export default function Me() {
    const user = useUser();
    const {data, error, isLoading,isValidating} = useSWR(()=>`/api/channel-info?mode=followers&channelid=${user.channelId}`,swrFetcher,{ refreshInterval: 3000 })
    const {data:meSubscribeData,isLoading:meSubscribeDataIsLoading, isValidating:meSubscribeDataIsValidating} = useSWR(user && "/api/subscribe?need_more_info=y",swrFetcher)
        ...
        <>
            <Modal isOpen={isOpen} onOpenChange={onOpenChange}>
                <ModalContent>
                    {(onClose) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1">정말로 프로필을 {isPublic ? "비공개로 전환":"공개"}할까요?</ModalHeader>
                            <ModalBody>
                                {
                                    isPublic ? <>
                                        <p>
                                            아래 버튼을 클릭하면 프로필이 다시 비공개상태가 되고, 아무도 '<code>{user.channelName}</code>'님의 프로필을 검색과 열람을 할 수 없어요.
                                        </p>
                                        <p>
                                        ...
                                        <p>
                                            아래 버튼을 클릭하면 프로필이 공개상태가 되고, 누구나 '<code>{user.channelName}</code>'님의 프로필을 검색과 열람을 할 수 있어요.
                                        </p>
                                        <p>
                            ...
                            <ModalFooter>
                                ...
                                <Button color="primary" onPress={async ()=>{
                                    handleChangePublicMode(!data?.isPublic).then(()=>onClose())
                ...
                <div className="w-[60%] shadow-2xl flex flex-col items-center justify-center gap-4 p-5 rounded-3xl dark:shadow-sky-950 dark:bg-gray-700">
                    ...
                    <div className="h-1 w-full rounded-3xl bg-gray-500"/>
                    {
                        !user || (error || isLoading) ? (
                            ...
                            <Avatar src={user?.channelImageUrl} className="w-32 h-32" showFallback/>
                    ...
                    {
                        meSubscribeData?.length > 0 ? <div className="w-full flex flex-wrap items-center gap-1">
                            {
                    ...
                    <Button color={isPublic ? "warning":"primary"} onPress={onOpen} fullWidth isLoading={isOpen} className="text-white font-extrabold">
                        {
                            isPublic ? "프로필 비공개 전환하기"
                                : "프로필 공개하기"
                        }
                    </Button>
                </div>
