...
export default async function handler(req, res) {
    const { mode, channelid, num=1,channelname='' } = req.query;
    if (!channelid) return res.status(403).send("필수 파라미터가 존재하지 않습니다.")
    ...
    try{
        ...
        if (error) {
            ...
            if (getChannelResp.data.length === 0) return res.status(404).send('No channel found')
